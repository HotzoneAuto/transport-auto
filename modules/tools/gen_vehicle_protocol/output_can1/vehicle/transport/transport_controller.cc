...
#include "modules/canbus/vehicle/transport/transport_controller.h"
...
bool TransportController::Start() {
  if (!is_initialized_) {
    AERROR << "TransportController has NOT been initiated.";
    return false;
  }
...
void TransportController::Stop() {
  if (!is_initialized_) {
    AERROR << "TransportController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "TransportController stopped.";
  }
}

Chassis TransportController::chassis() {
  chassis_.Clear();
...
void TransportController::Emergency() {
  set_driving_mode(Chassis::EMERGENCY_MODE);
  ResetProtocol();
}

ErrorCode TransportController::EnableAutoMode() {
...
ErrorCode TransportController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode TransportController::EnableSteeringOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
ErrorCode TransportController::EnableSpeedOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
void TransportController::Gear(Chassis::GearPosition gear_position) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
// -> pedal
void TransportController::Brake(double pedal) {
...
// gas:0.00~99.99 unit:
void TransportController::Throttle(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
void TransportController::Acceleration(double acc) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE ||
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
// transport default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:, left:-, right:+
void TransportController::Steer(double angle) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
...
void TransportController::Steer(double angle, double angle_spd) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
...
void TransportController::SetEpbBreak(const ControlCommand& command) {
...
void TransportController::SetBeam(const ControlCommand& command) {
...
void TransportController::SetHorn(const ControlCommand& command) {
...
void TransportController::SetTurningSignal(const ControlCommand& command) {
  // Set Turn Signal
  /* ADD YOUR OWN CAR CHASSIS OPERATION
...
void TransportController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool TransportController::CheckChassisError() {
  /* ADD YOUR OWN CAR CHASSIS OPERATION
  */
  return false;
}

void TransportController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR
          << "Too much time consumption in TransportController looping process:"
          << elapsed.count();
    }
  }
}

bool TransportController::CheckResponse(const int32_t flags, bool need_wait) {
  /* ADD YOUR OWN CAR CHASSIS OPERATION
  */
  return false;
}

void TransportController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t TransportController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode TransportController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void TransportController::set_chassis_error_code(
