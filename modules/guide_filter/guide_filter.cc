...
bool guide_Filter::Init() {
  ...
  chassis_detail_origin_reader_ = node_->CreateReader<ChassisDetail>(
      ...
      [this](const std::shared_ptr<ChassisDetail>& msg) {
        UpdateChassisDetail(msg);
      });
...
void guide_Filter::UpdateChassisDetail(
    const std::shared_ptr<ChassisDetail>& msg) {
  chassis_detail_ = *msg;
  ...
  // Follower Yaw rate
  static AverageFilter<double> follower_yaw_rate_filter(FilterLength, -180,
                                                        180);
  ...
  leader_acc_pedal_filter.AddElement(msg->leader_acc_pedal() +
                                     LeaderAccPedalCorrect);
  ...
  leader_brake_pedal_filter.AddElement(msg->leader_brake_pedal() +
                                       LeaderBrakePedalCorrect);
  ...
  if (f.is_open()) {
    ...
    while (!f.eof()) {
      ...
      if (SettingName == "FilterLength") {
        ...
        AINFO << "LeaderBrakePedalCorrect="
              << configinfo.LeaderBrakePedalCorrect;
